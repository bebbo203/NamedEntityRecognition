...
class Trainer():
    ...
    def train(self, train_dataset:Dataset, 
        ...
        for epoch in range(epochs):
            ...
            valid_loss, micro_precision, macro_precision, recall, f1 = self.evaluate(valid_dataset)
            torch.save(self.model.state_dict(), "model/inter_weights.pt")
           
            print('\t\t valid loss = {:0.4f}'.format(valid_loss))
            print('\t\t micro_precision = {:0.4f}'.format(micro_precision))
            print('\t\t macro_precision = {:0.4f}'.format(macro_precision))
            print('\t\t recall = {:0.4f}'.format(recall))
            print('\t\t f1 = {:0.4f}'.format(f1))

            fx = open(output_file, "a+")
            fx.write("%f, %f, %f, %f, %f, %f\n" % (avg_epoch_loss, valid_loss, micro_precision, macro_precision, recall, f1))
            fx.close()
